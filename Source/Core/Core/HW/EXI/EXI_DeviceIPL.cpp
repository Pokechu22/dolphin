...
bool CEXIIPL::LoadFontsFromAnyIPL()
{
  ...
  bool read_jis = LoadFileToIPL(ipl_rom_path, ROM_SHIFT_JIS_FONT_START,
...
bool CEXIIPL::LoadFileToIPL(const std::string& filename, u32 offset, u32 file_offset, u64 size)
{
  ...
  if (!stream.Seek(file_offset, SEEK_SET))
    return false;
...
#define DEV_ADDR_CURSOR(x) (DEV_ADDR(x) + m_cursor++)
    ...
    else if (IN_RANGE(WII_RTC))
    {
      ...
      {
        if (m_command.is_write())
          WARN_LOG(EXPANSIONINTERFACE, "Unknown Wii RTC write %02x with offset %x", data,
                   DEV_ADDR(WII_RTC));
        else
          WARN_LOG(EXPANSIONINTERFACE, "Unknown Wii RTC read with offset %x", DEV_ADDR(WII_RTC));
      }
    }

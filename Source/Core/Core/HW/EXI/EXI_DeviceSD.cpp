...
{
CEXISD::CEXISD(int channel_num)
{
  ASSERT_MSG(EXPANSIONINTERFACE, 0 <= channel_num && channel_num <= 2,
             "Trying to create invalid SD card index %d.", channel_num);
  ...
  INFO_LOG(EXPANSIONINTERFACE, "Reading SD card %s", filename.c_str());
  ...
  else
  {
    ...
    else if (command_position < 6)
    {
      ...
      if (command_position == 6)
      {
        ...
        if ((byte & 1) == 0)
        {
          WARN_LOG(EXPANSIONINTERFACE, "EXI SD command invalid, missing end bit: got %02x", byte);
...
void CEXISD::HandleCommand(Command command, u32 argument)
{
  ...
  case Command::SendOpCond:
  {
    ...
    INFO_LOG(EXPANSIONINTERFACE, "SD Host Capacity Support: %s", hcs ? "true" : "false");
...
u8 CEXISD::ReadForBlockRead()
{
  ...
  case BlockState::Token:
    ...
    {
      ERROR_LOG(EXPANSIONINTERFACE, "SD read failed at %08" PRIx64 " - error: %i, eof: %i", address,
  ...
  default:
    ERROR_LOG(EXPANSIONINTERFACE, "Unexpected block_state %d for reading", block_state);
    return 0xff;
  }
}
...
void CEXISD::WriteForBlockRead(u8 byte)
{
  if (byte != 0xff)
  {
    WARN_LOG(EXPANSIONINTERFACE, "Data written during block read: %02x, %d, %d", byte, block_state,
             block_position);
  }
  ...
  {
    INFO_LOG(EXPANSIONINTERFACE, "Assuming stop transmission");
    // Can just be done; no block is currently being transmitted
    if (block_state == BlockState::Token)
    {
      INFO_LOG(EXPANSIONINTERFACE, "In token state, marking as finished");
      address = 0;
      block_position = 0;
      block_state = BlockState::Nothing;
      state = State::ReadyForCommand;
    ...
    {
      INFO_LOG(EXPANSIONINTERFACE, "Changing from multi to single read to finish");
      state = State::SingleBlockRead;
    }
    else
    {
      INFO_LOG(EXPANSIONINTERFACE, "Already single read; doing nothing");
    }
...
u8 CEXISD::ReadForBlockWrite()
{
  ...
  case BlockState::ChecksumWritten:
  {
    ...
    {
      ERROR_LOG(EXPANSIONINTERFACE, "Bad CRC: was %04x, should be %04x", actual_crc, block_crc);
      result = DATA_RESPONSE_BAD_CRC;
    }
    else if (!m_card.Seek(address, SEEK_SET))
    {
      ERROR_LOG(EXPANSIONINTERFACE, "fseeko failed WTF");
    ...
    {
      ERROR_LOG(EXPANSIONINTERFACE, "SD write failed at %08" PRIx64 " - error: %i, eof: %i",
                address, ferror(m_card.GetHandle()), feof(m_card.GetHandle()));
      result = DATA_RESPONSE_WRITE_ERROR;
    }
    else
    {
      INFO_LOG(EXPANSIONINTERFACE, "SD write succeeded at %08" PRIx64, address);
  ...
  default:
    ERROR_LOG(EXPANSIONINTERFACE, "Unexpected block_state %d for writing", block_state);
    return 0xff;
  }
}
...
void CEXISD::WriteForBlockWrite(u8 byte)
{
  ...
  case BlockState::Token:
    ...
    {
      WARN_LOG(EXPANSIONINTERFACE, "Unexpected token for block write %02x", byte);
    }
    break;
  case BlockState::Block:

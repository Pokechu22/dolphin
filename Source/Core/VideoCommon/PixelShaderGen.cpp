...
uint WrapCoord(int coord, uint wrap, int size) {{
  switch (wrap) {{
    ...
    case {:s}:
      return uint(FlooredModulo(coord, size));  // coord % size
    case {:s}:
      if (FlooredModulo(coord, 2 * size) >= size) {{  // coord % (2 * size)
        coord = ~coord;
      }}
      return uint(FlooredModulo(coord, size));  // coord % size
  }}
...
uint WrapCoord(int coord, uint wrap, int size) {{
  ...
  if (!host_config.manual_texture_sampling)
  {
    out.Write("  float size_s = float(" I_TEXDIMS "[texmap].x) * 128;\n"
              ...
              "  float3 coords = float3(uv.x / size_s, uv.y / size_t, layer);\n");
    if (api_type == APIType::OpenGL || api_type == APIType::Vulkan)
    {
  ...
  int size_t = )" I_TEXDIMS R"([texmap].y;
    ...
    if (api_type == APIType::OpenGL || api_type == APIType::Vulkan)
      out.Write(R"(
  float2 uv_delta_x = abs(dFdx(uv));
  float2 uv_delta_y = abs(dFdy(uv));
)");
    else if (api_type == APIType::D3D)
      out.Write(R"(
  float2 uv_delta_x = abs(ddx(uv));
  float2 uv_delta_y = abs(ddy(uv));
)");
  ...
  int lod = max_delta == 0 ? min_lod : int(floor(log2(max_delta) * 16)) + (lod_bias >> 4);
